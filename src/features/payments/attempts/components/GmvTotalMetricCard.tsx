...
export const GmvTotalMetricCard = ({
  ...
  return (
    <div className={claseTarjeta(variante, "flex h-full min-h-44 flex-col p-4 text-center shadow-sm sm:p-5")}>
      <div className="mb-4 grid min-h-10 grid-cols-[2.25rem_minmax(0,1fr)_2.25rem] items-start gap-3">
        <span aria-hidden="true" />
        <p className={`min-w-0 text-xs font-medium leading-snug sm:text-sm ${esInvertida ? "text-dashboard-inverted-foreground" : "text-foreground"}`}>
          {etiqueta}
        </p>
        <span aria-hidden="true" />
      </div>
      <div className="flex flex-1 flex-col items-center justify-center">
        <p
          className={cn(
            "mb-3 w-full whitespace-normal break-words text-center text-lg font-bold leading-tight tracking-tight sm:text-xl xl:text-2xl",
        ...
        <div
          className={`inline-flex min-h-7 max-w-full items-center justify-center gap-1.5 rounded px-2 py-1 text-center text-[11px] leading-tight ${
